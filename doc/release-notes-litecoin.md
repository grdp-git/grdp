Groundpluscoin Core version *0.15.1* is now available from:

  <https://download.grdp.org/grdp-0.15.1/>
...
A number of changes to the way Groundpluscoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 grdps per kB (0.00001 GRDP). This is to prepare for dropping the minimum transaction fee to 0.1 grdps per kB (0.0001 GRDP) in the next release.
